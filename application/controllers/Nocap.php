...
class Nocap extends CI_Controller
{
	...
	function raport($id, $ta, $sms)
	{
		...
		$dt_muridin = $this->db->get_where('tbl_pengguna', ['pengguna_username' => $id])->row_array();
		$tgl_muridin = $this->ttl(date("Y-m-d", strtotime($dt_muridin['pengguna_register'])));
		// var_dump(year($muridditerima['pengguna_register']));
		// var_dump(date("d-m-y", strtotime($muridditerima['pengguna_register'])));
		// exit();
		//load data nilai
		...
		foreach ($result->result_array() as $res) {
			...
			// $diterima = $this->ttl($raportkeluar['tgl_dikeluarkan']);
			// DATA LEMBAGA
			...
			// $fpdf->Ln(40);
			$fpdf->SetX(120);
			$fpdf->Cell(40, 6, $kab_lembaga . ', ' . $tgl_muridin, 0, 1, 'L');
			$fpdf->SetX(120);
