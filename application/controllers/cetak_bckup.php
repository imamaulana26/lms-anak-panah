...
color: red;'>OPPS... ADA KESALAHAN... KEMBALI KE HALAMAN SEBELUMNYA...</h1>";
		...
		$raportkeluar = $this->db->get_where('tbl_thn_ajaran', ['thn_ajaran'=>str_replace('-', '/', $ta),'semester'=>$sms])->row_array();
		...
		if (empty($dt_kelas_a['kelas_nama'])) {
			echo "<h1 style='text-align: center;
margin-top: 50px;
color: red;'>NILAI RAPOR ANDA BELUM DI INPUT ADMIN</h1>";
			exit();
		}
		...
		// load data catatan
		$catatan_2 = $this->db->get_where('tbl_catmur', ['ta'=>str_replace('-', '/', $ta),'semester'=>2])->row_array();
...
// 			}
			...
			if ($res['siswa_alamat']!= null) $almt = $res['siswa_alamat']; else $almt = '-';
			$exp = explode(' ', $almt);
			// $itung = strlen($almt);
			// var_dump($itung);

			if ($res['siswa_no_telp']!= null) $notelp = $res['siswa_no_telp']; else $notelp = '-';
			if ($res['sekolah_asal']!= null) $sekolah_asal = $res['sekolah_asal']; else $sekolah_asal = '-';
			if ($res['ayah_nama']!= null) $nmayah = $res['ayah_nama']; else $nmayah = '-';
			if ($res['ibu_nama']!= null) $nmibu = $res['ibu_nama']; else $nmibu = '-';
			if ($res['wali_nama']!= null) $nmwali = $res['wali_nama']; else $nmwali = '-';
			if ($res['ayah_pekerjaan']!= null) $pkayah = $res['ayah_pekerjaan']; else $pkayah = '-';
			if ($res['ibu_pekerjaan']!= null) $pkibu = $res['ibu_pekerjaan']; else $pkibu = '-';
			if ($res['no_telp_ibu']!= null) $notelpibu = $res['no_telp_ibu']; else $notelpibu = '-';
			if ($res['no_telp_ayah']!= null) $notelpayah = $res['no_telp_ayah']; else $notelpayah = '-';
			if ($res['wali_alamat']!= null) $almt_wali = $res['wali_alamat']; else $almt_wali = '-';
			$exp_wl = explode(' ', $almt_wali);
			if ($res['wali_notelp']!= null) $notelp_wali = $res['wali_notelp']; else $notelp_wali = '-';
			if ($res['wali_pekerjaan']!= null) $pkwali = $res['wali_pekerjaan']; else $pkwali = '-';
			...
			if (!empty($catatan['catatan_siswa'])) $cat = $catatan['catatan_siswa']; else $cat = '-';
			if (!empty($catatan_2['catatan_siswa'])) $cat_2 = $catatan_2['catatan_siswa']; else $cat_2 = '-';

			// END OF DATA NILAI

			// DATA ABSENSI

			if (!empty($abs['sakit'])) $sakit = $abs['sakit']; else $sakit = '-';
			if (!empty($abs['izin'])) $izin = $abs['izin']; else $izin = '-';
			if (!empty($abs['tanpa_ket'])) $tanpaket = $abs['tanpa_ket']; else $tanpaket = '-';
			if (!empty($abs_2['sakit'])) $sakit_2 = $abs_2['sakit']; else $sakit_2 = '-';
			if (!empty($abs_2['izin'])) $izin_2 = $abs_2['izin']; else $izin_2 = '-';
			if (!empty($abs_2['tanpa_ket'])) $tanpaket_2 = $abs_2['tanpa_ket']; else $tanpaket_2 = '-';
			// END OF DATA ABSENSI

			// DATA CATATAN PENTING

			if (!empty($catpen['sikap'])) $sikap = $catpen['sikap']; else $sikap = '-';
			if (!empty($catpen['kegiatan'])) $kegiatan = $catpen['kegiatan']; else $kegiatan = '-';
			if (!empty($catpen['tugas'])) $tugas = $catpen['tugas']; else $tugas = '-';
			if (!empty($catpen_2['sikap'])) $sikap_2 = $catpen_2['sikap']; else $sikap_2 = '-';
			if (!empty($catpen_2['kegiatan'])) $kegiatan_2 = $catpen_2['kegiatan']; else $kegiatan_2 = '-';
			if (!empty($catpen_2['tugas'])) $tugas_2 = $catpen_2['tugas']; else $tugas_2 = '-';
			// END OF DATA CATATAN PENTING

			// WALIKELAS
			$wakel = $res['wali_kelas'];
			...
			for ($i=0; $i <3 ; $i++) { 
				...
				if (!empty($nilaiindiv[$i]['kegiatan'])) $keg = $nilaiindiv[$i]['kegiatan']; else $keg = '-';
				if (!empty($nilaiindiv[$i]['nilai'])) $nil = $nilaiindiv[$i]['nilai']; else $nil = '-';
				if ($nil<=0) $ket = '-'; else if ($nil<=85) $ket = 'Memuaskan';else $ket = 'Sangat Memuaskan';
		...
		$fpdf->Cell(60, 5, $kab_lembaga.', '.$dikel, 0, 1, 'L');
		$fpdf->SetX(15);
		...
		//WALIKELAS
		$fpdf->Cell(50, 30, '', 0, 1, 'L');
		$fpdf->SetX(140);
		$fpdf->Cell(60, 5, $wakel, 0, 1, 'L');

		if ($sms==2) {
		    if (empty($dt_kelas_a_2['kelas_nama'])) {
			echo "<h1 style='text-align: center;
margin-top: 50px;
color: red;'>NILAI RAPOR ANDA BELUM DI INPUT ADMIN</h1>";
			exit();
		...
		// CATATAN PENILAIAN PENTING
			...
			for ($i=0; $i <3 ; $i++) { 
				...
				if (!empty($nilaiindiv_2[$i]['kegiatan'])) $keg_2 = $nilaiindiv_2[$i]['kegiatan']; else $keg_2 = '-';
				if (!empty($nilaiindiv_2[$i]['nilai'])) $nil_2 = $nilaiindiv_2[$i]['nilai']; else $nil_2 = '-';
				if ($nil_2<=0) $ket_2 = '-'; else if ($nil_2<=85) $ket_2 = 'Memuaskan';else $ket_2 = 'Sangat Memuaskan';
				$fpdf->Cell(3, 5, '', 'LB', 0, 'C');
				$fpdf->Cell(2, 5, $i+1, 'B', 0, 'L');
				$fpdf->Cell(3, 5, '', 'B', 0, 'C');
