...
<head>
  ...
  <title>PKBM Anak Panah | Online Class</title>
      ...
      <section class="content">
        <div class="row">
          <div class="col-xs-12">
            <div class="box">
              <div class="box-body">
                <a href="<?= site_url('siswa') ?>" class="btn btn-info" style="float: right; margin-bottom: 10px">Kembali</a>
                <form id="fm_class_online" action="<?= site_url('siswa/save_online_class') ?>" method="post">
                  <label>Daftar Peserta Kelas Online</label>
                  <select class="js-example-basic-multiple col-md-12" name="peserta[]" multiple="multiple">
                    <?php foreach ($li_peserta as $key => $val) { ?>
                      <option value="<?= $val['siswa_nis'] ?>" <?= $val['oc'] == 1 ? 'selected' : '' ?>><?= $val['siswa_nama'] ?></option>
                    <?php } ?>
                  </select>
