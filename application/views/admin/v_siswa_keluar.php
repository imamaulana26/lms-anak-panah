...
<head>
  ...
  <title>PKBM Anak Panah | Data Siswa Keluar</title>
          ...
          <?php if ($c['pengguna_level']==1): ?>
            ...
            <li class="treeview active">
              ...
              <ul class="treeview-menu">
                ...
                <li class="active"><a href="<?php echo base_url().'siswa_keluar'?>"><i class="fa fa-star-o"></i> PD Keluar</a></li>
              </ul>
            </li>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    table = $('#table').DataTable({
      ...
      'order': [],
      'ajax': {
        'url': "<?= site_url('siswa_keluar/list_siswa_keluar') ?>",
        'type': "POST"
      },
      'columnDefs': [{
        'targets': [0, -1],
  ...
  function delete_siswa(id, nama){
    var del = confirm("Apakah anda yakin ingin menghapus semua data terkait siswa "+nama+" ?");
    if (del){
      document.location.href = "<?= site_url('siswa_keluar/delete_siswa') ?>"+"/"+id;
    }
  }
  ...
  function edit_siswa($nis){
    $.ajax({
      url: "<?= site_url('siswa/edit_siswa') ?>"+'/'+$nis,
      type: "post",
      dataType: "json",
...
$('#nama').val(data[0].siswa_nama);
  ...
  function detail_siswa(id){
    $("#table2").fadeIn(500);
    $("html, body").animate({ scrollTop: $("#table2").offset().top }, 500);
    $.ajax({
      ...
      success: function(data){
        ...
        html += '<tr><td style="width:15%;"> Jenis Kelamin</td><td style="width:1%;">:</td><td>'+data.siswa_jenkel+'</td></tr>';
        ...
        html += '<tr><td style="width:15%;"> Status</td><td style="width:1%;">:</td><td>'+data.kelas_nama+'</td></tr>';
        html += '</table>';
        html += '<div class="box" >';
        ...
        html += '</tr>';
        html += '<tr>';
        html += '<td style="border-right:5px solid #ecf0f5"> Pendidikan Terakhir : '+data.ayah_pekerjaan+'</td>';
        html += '<td style="border-right:5px solid #ecf0f5"> Pendidikan Terakhir : '+data.ibu_pekerjaan+'</td>';
        html += '<td> Pendidikan Terakhir : '+data.wali_pekerjaan+'</td>';
        html += '</tr>';
        html += '<tr>';
        html += '<td style="border-right:5px solid #ecf0f5"> Pendidikan Terakhir : '+data.ayah_penghasilan+'</td>';
        html += '<td style="border-right:5px solid #ecf0f5"> Pendidikan Terakhir : '+data.ibu_penghasilan+'</td>';
        html += '<td> Pendidikan Terakhir : '+data.wali_penghasilan+'</td>';
        html += '</tr>';
        html += '<tr>';
        html += '<td colspan="3">  <button onclick="destroy_datatable()" class="btn btn-danger btn-flat" style="float: right;"><span class="fa fa-times"></span> Close</button></td>';
        html += '</tr>';
        html += '</table>';
        // html += "<table class='table table-striped table-bordered table-hover' >";
        // html += '</table>';
